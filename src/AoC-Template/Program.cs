...
internal class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        var rootCommand = new RootCommand { langOption, daysOption, };
    ...
    private static void RunWithBench(string lang, IEnumerable<int> days)
    {
        ...
        {
            List<(BaseChallengeFSharp challenge, int)> challenges = BulkBenchFSharp
                .Challenges()
                .Select(c => (c, int.Parse(c.DayIdentifier)))
                .ToList();
            foreach (var day in days)
            {
    ...
    private static void RunFSharpWithoutBench(IEnumerable<int> days)
    {
        foreach (var day in days)
        {
            var challenge = BulkBenchFSharp
                .Challenges()
                .FirstOrDefault(c => int.Parse(c.DayIdentifier) == day);
            if (challenge is null)
                Console.WriteLine($"No challenge found for day {day}.");
            else
            {
